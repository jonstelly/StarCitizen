        ...
        public async Task ValidateStarMapJson()
        {
            using (var client = new HttpClient())
            {
                using (var resp = await client.PostAsync(new Uri(BaseUrl, "bootup"), null))
                {
                    ...
                    var actual = JsonConvert.SerializeObject(JsonConvert.DeserializeObject(await resp.Content.ReadAsStringAsync()), Formatting.Indented);
                    var expected = JsonConvert.SerializeObject(JsonConvert.DeserializeObject(Encoding.UTF8.GetString(Json.ApiResponses.StarMapBootstrap)), Formatting.Indented);
                    if (actual != expected)
                    {
